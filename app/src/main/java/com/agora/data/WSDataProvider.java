...
public class WSDataProvider implements IDataProvider {
    ...
    public List<Category> fetchProductCategories() {
        JSONArray reader = HTTPJSONRequest.requestHttpGetArray(AppConfig.WS_URL + "categories");
        List<Category> list= new ArrayList<>();
        try {
            if (reader==null){
                //Mostrar erorr que no hay connection WS
            }else {
                 JSONArray array = reader;
    ...
    public List<Category> fetchSubCategories(Long categoryKey) {
        JSONArray reader = HTTPJSONRequest.requestHttpGetArray(AppConfig.WS_URL + "categories/" + categoryKey + "/subcategories");
        List<Category> list= new ArrayList<>();
        try {
            if (reader==null){
                //Mostrar erorr que no hay connection WS
    ...
    public List<Need> fetchNeedsByUserKey(Long userKey) {
        JSONArray reader = HTTPJSONRequest.requestHttpGetArray(AppConfig.WS_URL + "users/"+userKey+"/needs");
               ...
               JSONArray array = reader;
                for (int n = 0; n < array.length(); n++) {
                   JSONObject jsonNeed = array.getJSONObject(n);
                    ...
                    Bitmap image= HTTPRequest.requestImageHttpGet(AppConfig.ICONS_URL + need.getCategory().getImageName());
    ...
    public Long createDeal(Deal deal) {
        String jsonRq="{"+deal.populateJson().toString()+"}";
        jsonRq= jsonRq.replace("\n", " ").replace("\r", " ");
            JSONObject reader = HTTPJSONRequest.requestHttpPost(AppConfig.WS_URL +"needs/"+deal.getNeed().getNeedKey()+ "/deal", deal.populateJson().toString());
            try {
...
/*            JSONObject reader = HTTPJSONRequest.requestHttpGet(AppConfig.WS_URL + "bidsRead" + "?needKey=" + needKey);
    ...
    public Long createNeed(Need need, Long userKey) {
        String jsonRq="{"+need.populateJson().toString()+"}";
        jsonRq= jsonRq.replace("\n", " ").replace("\r", " ");
        JSONObject reader = HTTPJSONRequest.requestHttpPut(AppConfig.WS_URL + "users/"+userKey+"/need", need.populateJson().toString());
        try {
            Long key=reader.getLong("result");
    ...
    public void uploadImage(HashMap<String, String> params) {
        HTTPJSONRequest.uploadImage(AppConfig.WS_URL+"images/"+"uploadImageBase64",params);
    }
    ...
    public User signIn(Auth auth) {
        ...
        JSONObject o= auth.populateJson();
        JSONObject reader = HTTPJSONRequest.requestHttpPost(AppConfig.WS_URL + "users/signIn", o.toString());
        try {
            //JSONObject jsonUser = reader.getJSONObject("user");
            User user = (User) BuilderFactory.getBuilder(UserBuilder.class.getName()).build(reader);
            user.setAuth(auth);
            return user;
        } catch (JSONException e) {
            e.printStackTrace();
    ...
    public List<Bid> fetchBidByNeedKey(Long need) {
        JSONArray reader = HTTPJSONRequest.requestHttpGetArray(AppConfig.WS_URL+"needs/"+need+"/bids");
